...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet:
  ...
  override def mostRetweeted: Tweet =
    var most = elem
    this.foreach(x => {
      most = if x.retweets > most.retweets then x else most
    })
    most

  override def descendingByRetweet: TweetList =
    @tailrec
    def doWork(l: TweetSet, tl: TweetList): TweetList =
      l match {
        case _: Empty => tl
        case _ =>
          val t = l.mostRetweeted
          doWork(l.remove(t), tl.append(t))
      }
...
//    this match {
//      case  => new Cons(t, Nil)
//      case _ => new Cons(head, tail.append(t))
//    }
...
object Nil extends TweetList:
  ...
  override def append(t: Tweet): TweetList =
    new Cons(t, Nil)
...
object GoogleVsApple:
  ...
  lazy val googleTweets: TweetSet = allTweets.filter(x => google.exists(c => x.text.contains(c)))
  lazy val appleTweets: TweetSet = allTweets.filter(x => apple.exists(c => x.text.contains(c)))
