...
object Anagrams extends AnagramsInterface :
   ...
   */
  def wordOccurrences(w: Word): Occurrences = w.toLowerCase().groupBy(x => x).toList.sorted.map(x => (x._1, x._2.length))
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences =
    y.foldLeft(x.toMap) { case (acc, (char, count)) =>
      if acc(char) == count then acc - char else acc + ((char, acc(char) - count))
    }.toList
   ...
   * Combinations
   * List(List('a', 1), List(('a', 1), ('e', 1)), List(('a', 1), ('e', 1), ('m', 1))...)
   */
